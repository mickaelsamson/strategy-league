...
function submitMove(lineIndex){
  ...
  socket.emit("moonveil_glyph_move", { ...selected, lineIndex });
  selected = null;
}
...
function render(){
  if(!state) return;
  document.body.classList.toggle("moonveil_glyph-scoring", state.phase === "scoring");
  document.body.dataset.moonveil_glyphPlayers = String(state.players?.length || 2);
  renderStatus();
  renderFactories();
  renderCenter();
  renderPlayers();
...
async function showRoundScorePopups(){
  ...
  return new Promise(resolve => requestAnimationFrame(async () => {
    document.body.classList.add("moonveil_glyph-scoring");
    for(const player of (state.players || [])){
      ...
      for(const placement of (summary.placements || [])){
        if(token !== scoringToken){
          ...
          document.body.classList.remove("moonveil_glyph-scoring");
          return resolve();
        }
...
function resign(){
  ...
  if(tutorialMode){
    gameOver = true;
    state.ended = true;
    tutorialGuide?.message("Tutorial resigned. Return to the lobby and restart when you want another guided table.", ["In real Moonveil Glyph, resigning is rarely needed: floor penalties can be recovered with bonuses."]);
    return;
  }
  socket.emit("moonveil_glyph_resign");
}
...
function startTutorialGame(){
  state = {
    gameId: "moonveil_glyph-tutorial",
    round: 1,
    phase: "playing",
    turnSeat: 0,
    mySeat: 0,
  ...
  tutorialGuide = tutorialGuide || window.TutorialGuide?.create({
    title: "Moonveil Glyph Tutorial",
    steps: TUTORIAL_STEPS,
    onBack: () => window.location.href = "/moonveil-glyph/index.html"
  });
  render();
  startTimerLoop();
}
...
function finishTutorialMoonveilGlyph(){
  state.ended = true;
  gameOver = true;
...
socket.on("moonveil_glyph_state", data => {
...
socket.on("moonveil_glyph_end", async data => {
  gameOver = true;
  lastEndPayload = data;
...
socket.on("moonveil_glyph_rematch_status", data => {
...
socket.on("moonveil_glyph_notice", data => {
  const hint = document.getElementById("selectionHint");
  if(hint && data?.message) hint.innerText = data.message;
});

socket.on("moonveil_glyph_rematch_start", () => {
  gameOver = false;
  document.getElementById("endScreen").className = "end-screen";
  document.getElementById("rematchStatus").innerText = "";
  document.getElementById("finalScores").innerHTML = "";
