...
socket.on("moonveil_glyph_lobbies_update", lobbies => {
  ...
  if(!list.length){
    target.innerHTML = `<div class="empty-state">No Moonveil Glyph lobby yet. Open the first table.</div>`;
    return;
  }
...
function createLobby(){
  ...
  socket.emit("create_moonveil_glyph_lobby", {
...
function publicMatchmaking(){
  if(!gamesEnabled) return;
  socket.emit("public_moonveil_glyph_matchmaking", {
    maxPlayers: Number(document.getElementById("maxPlayers").value)
  });
}
...
function join(id){
  if(!gamesEnabled) return;
  socket.emit("join_moonveil_glyph_lobby", id);
}

function ready(id){
  if(!gamesEnabled) return;
  socket.emit("toggle_moonveil_glyph_ready", id);
}

function openInvitePicker(){
  if(!gamesEnabled) return;
  window.SiteShell?.openInvitePicker?.("moonveil_glyph");
}

function inviteToLobby(id){
  if(!gamesEnabled) return;
  window.SiteShell?.openLobbyInvitePicker?.("moonveil_glyph", id);
}

async function checkGamesAccess(){
  try{
    const res = await fetch("/api/games/status?gameKey=moonveil_glyph");
...
socket.on("moonveil_glyph_start", () => {
  window.location = "/moonveil-glyph/game.html";
});
