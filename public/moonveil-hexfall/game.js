(() => {
  ...
  function syncOnlineState(){
    ...
    socket.emit('moonveil_hexfall_sync_state', {
        ...
        hex.addEventListener('click', () => {
          if(isOnlineGame()){
            ...
            if(isHost()){
              ...
              socket.emit('moonveil_hexfall_action', { gameId: online.gameId, action: { type: 'play', r, c } });
            }
            return;
          }
  ...
  function showWin(forcedWinner = null, surrendered = false){
    ...
    window.dispatchEvent(new CustomEvent('moonveil_hexfall:gameover', {
      detail: {
  ...
  function startTutorialGame(){
    ...
    tutorialGuide = tutorialGuide || window.TutorialGuide?.create({
      title: 'Moonveil Hexfall Tutorial',
      steps: TUTORIAL_STEPS,
      onBack: () => window.location.href = '/moonveil-hexfall/index.html'
    });
    startGameView();
    updateTutorialGuide();
  }
  ...
  function renderOnlineLobbies(){
    ...
    if(pendingInviteLobbyId){
      ...
      if(invited && !already && !full){
        socket?.emit('join_moonveil_hexfall_lobby', pendingInviteLobbyId);
      ...
      return `
        <article class="sl-lobby-card">
          <div class="sl-lobby-head">
            <div><strong>${escapeHtml(lobby.name)}</strong><span>${lobby.matchmaking === 'public' ? 'Public matchmaking' : 'Moonveil Hexfall duel'}</span></div>
    ...
    dom.onlineLobbyList.querySelectorAll('[data-action]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if(!socket || !id) return;
        if(button.dataset.action === 'join') socket.emit('join_moonveil_hexfall_lobby', id);
        if(button.dataset.action === 'ready') socket.emit('toggle_moonveil_hexfall_ready', id);
        if(button.dataset.action === 'leave') socket.emit('leave_moonveil_hexfall_lobby', id);
        if(button.dataset.action === 'invite') window.SiteShell?.openLobbyInvitePicker?.('moonveil_hexfall', id);
      });
    });
  }
  ...
  function registerSocket(){
    ...
    socket.on('moonveil_hexfall_lobbies_update', lobbies => {
      online.lobbies = lobbies || {};
      renderOnlineLobbies();
    });

    socket.on('moonveil_hexfall_notice', payload => {
      if(payload?.message) dom.status.textContent = payload.message;
    });

    socket.on('moonveil_hexfall_start', payload => {
    ...
    socket.on('moonveil_hexfall_state', payload => {
      if(payload?.gameId && online.gameId && payload.gameId !== online.gameId) return;
      applySnapshot(payload?.snapshot);
    });

    socket.on('moonveil_hexfall_action_request', payload => {
      if(!isHost()) return;
  ...
  function initEvents(){
    ...
    dom.createOnlineLobbyBtn.addEventListener('click', () => {
      if(!socket) return;
      socket.emit('create_moonveil_hexfall_lobby', { name: dom.onlineLobbyName.value, maxPlayers: 2 });
    });

    dom.publicMatchmakingBtn?.addEventListener('click', () => {
      if(!socket) return;
      socket.emit('public_moonveil_hexfall_matchmaking', { maxPlayers: 2 });
    });
    ...
    dom.resetBtn.addEventListener('click', () => {
      if(isOnlineGame()){
        ...
        if(isHost()){
          ...
          socket.emit('moonveil_hexfall_action', { gameId: online.gameId, action: { type: 'reset' } });
        }
        return;
      }
    ...
    dom.surrenderBtn.addEventListener('click', () => {
      ...
      if(isOnlineGame()){
        ...
        if(isHost()){
          ...
          socket.emit('moonveil_hexfall_action', { gameId: online.gameId, action: { type: 'surrender' } });
        }
        return;
      }
