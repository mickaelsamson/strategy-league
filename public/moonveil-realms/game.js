(() => {
  ...
  const RESOURCES = {
    cedar: { name: 'Wood', short: 'Wo', color: '#78d6a7', tile: '#1f6b40', dark: '#0b251d', icon: '/moonveil-realms/assets/generated/res-wood-512.png', texture: '/moonveil-realms/assets/generated/tile-forest.png' },
    clay: { name: 'Brick', short: 'Br', color: '#e08a58', tile: '#8a3f2c', dark: '#26100d', icon: '/moonveil-realms/assets/generated/res-brick-512.png', texture: '/moonveil-realms/assets/generated/tile-brick.png' },
    rice: { name: 'Wheat', short: 'Wh', color: '#f1d274', tile: '#a37b24', dark: '#2d210b', icon: '/moonveil-realms/assets/generated/res-wheat-512.png', texture: '/moonveil-realms/assets/generated/tile-wheat.png' },
    wisteria: { name: 'Sheep', short: 'Sh', color: '#9bc66b', tile: '#415e2a', dark: '#17210e', icon: '/moonveil-realms/assets/generated/res-sheep-512.png', texture: '/moonveil-realms/assets/generated/tile-pasture.png' },
    sunsteel: { name: 'Ore', short: 'Or', color: '#c7d2d9', tile: '#677173', dark: '#1d2427', icon: '/moonveil-realms/assets/generated/res-stone-512.png', texture: '/moonveil-realms/assets/generated/tile-mountain.png' }
  };

  const ASSETS = {
    background: '/moonveil-realms/assets/generated/moonveil-bg.png',
    crater: '/moonveil-realms/assets/generated/tile-crater.png',
    ship: '/moonveil-realms/assets/generated/port-ship-cutout.png',
    token: '/moonveil-realms/assets/generated/token-number-cutout.png',
    oni: '/moonveil-realms/assets/generated/token-oni-medallion.png',
    cardBack: '/moonveil-realms/assets/generated/card-back-512.png',
    cardKnight: '/moonveil-realms/assets/generated/card-knight-512.png',
    pieceRoad: '/moonveil-realms/assets/generated/piece-road-board.png',
    pieceSettlement: '/moonveil-realms/assets/generated/piece-settlement-board.png',
    pieceCity: '/moonveil-realms/assets/generated/piece-city-board.png',
    pieceDev: '/moonveil-realms/assets/generated/piece-dev-card-board.png',
    boardPieces: {
      roads: [
        '/moonveil-realms/assets/player-pieces/road-red.svg',
        '/moonveil-realms/assets/player-pieces/road-blue.svg',
        '/moonveil-realms/assets/player-pieces/road-green.svg',
        '/moonveil-realms/assets/player-pieces/road-gold.svg'
      ],
      settlements: [
        '/moonveil-realms/assets/player-pieces/settlement-red.svg',
        '/moonveil-realms/assets/player-pieces/settlement-blue.svg',
        '/moonveil-realms/assets/player-pieces/settlement-green.svg',
        '/moonveil-realms/assets/player-pieces/settlement-gold.svg'
      ],
      cities: [
        '/moonveil-realms/assets/player-pieces/city-red.svg',
        '/moonveil-realms/assets/player-pieces/city-blue.svg',
        '/moonveil-realms/assets/player-pieces/city-green.svg',
        '/moonveil-realms/assets/player-pieces/city-gold.svg'
      ]
    },
    crests: [
      '/moonveil-realms/assets/generated/crest-red-cutout.png',
      '/moonveil-realms/assets/generated/crest-blue-cutout.png',
      '/moonveil-realms/assets/generated/crest-green-cutout.png',
      '/moonveil-realms/assets/generated/crest-gold-cutout.png'
    ]
  };
  ...
  function renderOnlineLobbyList(){
    ...
    if(!onlineState.available){
      dom.onlineLobbyStatus.textContent = 'Open Moonveil through the app server to unlock online lobbies.';
    ...
    if(pendingInviteLobbyId){
      ...
      if(invited && !already && !full){
        socket?.emit('join_moonveil_realms_lobby', pendingInviteLobbyId);
    ...
    if(!lobbies.length){
      ...
      dom.onlineLobbyMeta.textContent = onlineState.currentLobbyId ? 'Everyone must ready up to enter Moonveil.' : 'Online Moonveil supports 2 to 4 human players.';
      dom.onlineLobbyList.innerHTML = '<div class="online-empty">No Moonveil lobby yet. Open the first table.</div>';
      return;
    }
    ...
    dom.onlineLobbyList.querySelectorAll('[data-online-action]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if(action === 'join') socket?.emit('join_moonveil_realms_lobby', lobbyId);
        if(action === 'leave') socket?.emit('leave_moonveil_realms_lobby', lobbyId);
        if(action === 'ready') socket?.emit('toggle_moonveil_realms_ready', lobbyId);
        if(action === 'invite') window.SiteShell?.openLobbyInvitePicker?.('moonveil_realms', lobbyId);
      });
    });
  }
    ...
    if(lobby){
      updateOnlineStatus(
        ...
        lobby.players.every(player => player.ready)
          ? 'All clans are ready. Entering Moonveil...'
      ...
      updateOnlineStatus('Choose a lobby or create one.', 'Online Moonveil supports 2 to 4 human players.');
    }
    renderOnlineLobbyList();
  }
  ...
  function syncOnlineState(){
    ...
    socket.emit('moonveil_realms_sync_state', {
  ...
  function requestOnlineAction(action){
    ...
    socket.emit('moonveil_realms_action', {
      gameId: onlineState.gameId,
      action
    });
  }
  ...
  function startLocalGame(event){
    ...
    logLocal(nextState, 'The clans enter the Moonveil night.');
    enterGame(nextState);
  ...
  function startTutorialGame(){
    ...
    logLocal(nextState, 'Tutorial begins: build a Moonveil economy faster than the AI clans.');
    tutorialGuide = tutorialGuide || window.TutorialGuide?.create({
      title: 'Moonveil Realms Tutorial',
      steps: TUTORIAL_STEPS,
      onBack: () => window.location.href = '/moonveil-realms/index.html'
    });
    enterGame(nextState);
  ...
  function checkVictory(playerId){
    ...
    if(victoryPoints(player) >= state.targetScore){
      ...
      state.phase = 'ended';
      state.activeMode = null;
      log(`${player.name} wins Moonveil Realms.`);
  ...
  function applyPlayerSurrender(playerId, reason = 'surrender'){
    ...
    if(remainingHumans.length <= 1){
      ...
      closeTradeResponse();
      closeTradeModal();
      log(`${player.name} surrenders. ${state.players[state.winner].name} wins Moonveil Realms.`);
  ...
  function updateTutorialGuide(){
    ...
    if(state.winner){
      tutorialGuide.complete(`${state.players[state.winner]?.name || 'A clan'} wins. You completed a guided Moonveil Realms game against AI clans.`);
    }
  }
  ...
  function bindSocket(){
    ...
    socket.on('moonveil_realms_lobbies_update', lobbies => {
    ...
    socket.on('moonveil_realms_start', payload => {
    ...
    socket.on('moonveil_realms_state', ({ snapshot, hostUsername, gameId } = {}) => {
      applySnapshot(snapshot, hostUsername, gameId);
    });

    socket.on('moonveil_realms_action_request', ({ username, action } = {}) => {
      applyOnlineAction(username, action);
    });

    socket.on('moonveil_realms_notice', ({ message } = {}) => {
      if(message) notice(message);
    });
  }
  ...
  dom.createOnlineLobbyBtn?.addEventListener('click', () => {
    if(!socket) return;
    socket.emit('create_moonveil_realms_lobby', {
  ...
  dom.publicMatchmakingBtn?.addEventListener('click', () => {
    if(!socket) return;
    socket.emit('public_moonveil_realms_matchmaking', {
      maxPlayers: Number(dom.onlineMaxPlayers.value) || 4,
      boardMode: dom.onlineBoardMode.value,
      targetScore: Number(dom.onlineTargetScore.value) || TARGET_DEFAULT,
      turnTimerSeconds: readTimerValue(dom.onlineTurnTimer)
