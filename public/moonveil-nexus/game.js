(() => {
  ...
  const dom = {
    ...
    boardFrame: document.querySelector('.moonveil-board-frame'),
  ...
  function syncOnlineState(){
    ...
    socket.emit('moonveil_nexus_sync_state', {
        ...
        cell.addEventListener('click', () => {
          if(isOnlineGame()){
            ...
            if(isHost()){
              ...
              socket.emit('moonveil_nexus_action', { gameId: online.gameId, action: { type: 'drop', col: c } });
            }
            return;
          }
  ...
  function renderCrest(target, info, token){
    ...
    const emblem = token === P1
      ? '/moonveil-nexus/assets/ui/moonveil-emblem.png'
      : '/moonveil-nexus/assets/ui/nexus-emblem.png';
  ...
  function showWin(forcedWinner = null, surrendered = false){
    ...
    window.dispatchEvent(new CustomEvent('moonveil:gameover', {
      detail: {
  ...
  function goToSetup(){
    ...
    document.body.classList.remove('is-playing-nexus');
  ...
  function startGameView(){
    ...
    dom.winCard.classList.add('hidden');
    document.body.classList.add('is-playing-nexus');
    window.scrollTo(0, 0);
  ...
  function startTutorialGame(){
    ...
    tutorialGuide = tutorialGuide || window.TutorialGuide?.create({
      ...
      onBack: () => window.location.href = '/moonveil-nexus/index.html'
    });
    startGameView();
    updateTutorialGuide();
  }
  ...
  function renderOnlineLobbies(){
    ...
    if(pendingInviteLobbyId){
      ...
      if(invited && !already && !full){
        socket?.emit('join_moonveil_nexus_lobby', pendingInviteLobbyId);
    ...
    dom.onlineLobbyList.querySelectorAll('[data-action]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if(!socket || !id) return;
        if(button.dataset.action === 'join') socket.emit('join_moonveil_nexus_lobby', id);
        if(button.dataset.action === 'ready') socket.emit('toggle_moonveil_nexus_ready', id);
        if(button.dataset.action === 'leave') socket.emit('leave_moonveil_nexus_lobby', id);
        if(button.dataset.action === 'invite') window.SiteShell?.openLobbyInvitePicker?.('moonveil_nexus', id);
      });
    });
  }
  ...
  function registerSocket(){
    ...
    socket.on('moonveil_nexus_lobbies_update', lobbies => {
      online.lobbies = lobbies || {};
      renderOnlineLobbies();
    });

    socket.on('moonveil_nexus_notice', payload => {
      if(payload?.message) dom.status.textContent = payload.message;
    });

    socket.on('moonveil_nexus_start', payload => {
    ...
    socket.on('moonveil_nexus_state', payload => {
      if(payload?.gameId && online.gameId && payload.gameId !== online.gameId) return;
      applySnapshot(payload?.snapshot);
    });

    socket.on('moonveil_nexus_action_request', payload => {
      if(!isHost()) return;
  ...
  function initEvents(){
    ...
    dom.createOnlineLobbyBtn.addEventListener('click', () => {
      ...
      socket.emit('create_moonveil_nexus_lobby', { name: dom.onlineLobbyName.value, maxPlayers: 2 });
    });

    dom.publicMatchmakingBtn?.addEventListener('click', () => {
      if(!socket || isGameDisabled()) return;
      socket.emit('public_moonveil_nexus_matchmaking', { maxPlayers: 2 });
    });
    ...
    dom.resetBtn.addEventListener('click', () => {
      if(isOnlineGame()){
        ...
        if(isHost()){
          ...
          socket.emit('moonveil_nexus_action', { gameId: online.gameId, action: { type: 'reset' } });
        }
        return;
      }
    ...
    dom.surrenderBtn.addEventListener('click', () => {
      ...
      if(isOnlineGame()){
        ...
        if(isHost()){
          ...
          socket.emit('moonveil_nexus_action', { gameId: online.gameId, action: { type: 'surrender' } });
        }
        return;
      }
      ...
      if(event.key === 'Enter' || event.key === ' ') {
        ...
        if(isOnlineGame()){
          ...
          if(isHost()){
            ...
            socket.emit('moonveil_nexus_action', { gameId: online.gameId, action: { type: 'drop', col } });
          }
        }else{
          if(tutorialMode && state.currentPlayer !== P1) return;
          applyDrop(col);
