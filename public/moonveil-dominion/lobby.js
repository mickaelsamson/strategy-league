...
socket.on("moonveil_dominion_lobbies_update",lobbies=>{
 ...
 return `<div class="sl-lobby-card">
  <div class="sl-lobby-head">
   <div><strong>${escapeHtml(l.name)}</strong><span>${l.matchmaking==="public"?"Public matchmaking":"Moonveil Dominion duel"}</span></div>
...
function createLobby(){
 ...
 socket.emit("create_moonveil_dominion_lobby",{name:document.getElementById("name").value});
}
function publicMatchmaking(){
 if(!gamesEnabled)return;
 socket.emit("public_moonveil_dominion_matchmaking");
}
function join(id){if(gamesEnabled)socket.emit("join_moonveil_dominion_lobby",id);}
function ready(id){if(gamesEnabled)socket.emit("toggle_moonveil_dominion_ready",id);}
function openInvitePicker(){
 if(!gamesEnabled)return;
 window.SiteShell?.openInvitePicker?.("moonveil_dominion");
}
function inviteToLobby(id){
 if(!gamesEnabled)return;
 window.SiteShell?.openLobbyInvitePicker?.("moonveil_dominion",id);
}

async function checkGamesAccess(){
 try{
 const res=await fetch("/api/games/status?gameKey=moonveil_dominion");
...
socket.on("moonveil_dominion_start",()=>window.location="/moonveil-dominion/game.html");
checkGamesAccess();
