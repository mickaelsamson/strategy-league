...
function createMoonveilNexusModule({ io, socket, state, updatePresence, isGameAllowed, applyStructuredGameResult }){
  function findLobbyByUsername(username){
    return Object.values(state.moonveilNexusLobbies).find(lobby =>
      lobby.players.some(player => player.username === username)
    ) || null;
  }

  function findGameByUsername(username){
    return Object.values(state.moonveilNexusGames).find(game =>
  ...
  function emitStart(game){
    ...
    game.players.forEach(player => {
      if(!player.id) return;
      io.sockets.sockets.get(player.id)?.emit('moonveil_nexus_start', {
        gameId: game.id,
  ...
  function emitState(game, targetSocketId = null){
    ...
    if(targetSocketId){
      io.sockets.sockets.get(targetSocketId)?.emit('moonveil_nexus_state', payload);
      return;
    }

    emitToGame(game, 'moonveil_nexus_state', payload);
  }
  ...
  function startLobbyGame(lobby){
    ...
    state.moonveilNexusGames[gameId] = game;
    delete state.moonveilNexusLobbies[lobby.id];

    game.players.forEach(player => {
      if(player.id) state.moonveilNexusPlayerGames[player.id] = gameId;
    });
  ...
  function findPublicMatch(maxPlayers){
    return randomEntry(Object.values(state.moonveilNexusLobbies).filter(lobby =>
  ...
  function handleLobbyDisconnect(socketId){
    for(const id in state.moonveilNexusLobbies){
      const lobby = state.moonveilNexusLobbies[id];
      lobby.players = lobby.players.filter(player => player.id !== socketId);
      if(!lobby.players.length){
        delete state.moonveilNexusLobbies[id];
        continue;
      }
  ...
  function rebindForUsername(username){
    ...
    if(player){
      if(player.id && player.id !== socket.id){
        delete state.moonveilNexusPlayerGames[player.id];
      }
      player.id = socket.id;
      state.moonveilNexusPlayerGames[socket.id] = game.id;
    }
  ...
  function handleDisconnect(){
    delete state.moonveilNexusPlayerGames[socket.id];
    handleLobbyDisconnect(socket.id);

    for(const game of Object.values(state.moonveilNexusGames)){
      const player = game.players.find(entry => entry.id === socket.id);
      if(player) player.id = null;

      if(game.hostSocketId === socket.id){
        game.hostSocketId = null;
        emitToGame(game, 'moonveil_nexus_notice', {
  ...
  function register(){
    socket.on('create_moonveil_nexus_lobby', ({ name, maxPlayers } = {}) => {
      if(isGameAllowed && !isGameAllowed('moonveil_nexus', socket)) return;
    ...
    socket.on('join_moonveil_nexus_lobby', id => {
      if(isGameAllowed && !isGameAllowed('moonveil_nexus', socket)) return;
      if(!socket.username) return;
      if(findLobbyByUsername(socket.username) || findGameByUsername(socket.username)) return;
    ...
    socket.on('leave_moonveil_nexus_lobby', id => {
      const lobby = state.moonveilNexusLobbies[id];
      if(!lobby) return;
    ...
    socket.on('toggle_moonveil_nexus_ready', id => {
      if(isGameAllowed && !isGameAllowed('moonveil_nexus', socket)) return;
      const lobby = state.moonveilNexusLobbies[id];
      if(!lobby) return;
    ...
    socket.on('public_moonveil_nexus_matchmaking', ({ maxPlayers } = {}) => {
      if(isGameAllowed && !isGameAllowed('moonveil_nexus', socket)) return;
      if(!socket.username) return;
      if(findLobbyByUsername(socket.username) || findGameByUsername(socket.username)) return;
    ...
    socket.on('moonveil_nexus_action', ({ gameId, action } = {}) => {
      if(!gameId || !action || !socket.username) return;
      const game = state.moonveilNexusGames[gameId];
      if(!game) return;
      const player = game.players.find(entry => entry.username === socket.username);
      if(!player) return;

      if(!game.hostSocketId){
        socket.emit('moonveil_nexus_notice', { message: 'The host is reconnecting. Please wait a moment.' });
        return;
      }

      const hostSocket = io.sockets.sockets.get(game.hostSocketId);
      if(!hostSocket){
        socket.emit('moonveil_nexus_notice', { message: 'The host is unavailable right now.' });
        return;
      }

      hostSocket.emit('moonveil_nexus_action_request', {
        gameId,
    ...
    socket.on('moonveil_nexus_sync_state', ({ gameId, snapshot } = {}) => {
      ...
      applyStructuredGameResult({
        ...
        .catch(err => {
          console.error('Moonveil Nexus result update error:', err);
        })
