...
function createMoonveilGlyphModule({ io, socket, state, updatePresence, applyMoonveilGlyphResult, isGameAllowed }){
  ...
  function endByTimeout(game){
    ...
    if(remaining.length > 1){
      ...
      game.players.forEach(player => {
        ...
        if(s) s.emit('moonveil_glyph_notice', { message: `${loser.username} ran out of time and was eliminated.` });
      });
      updatePresence();
      return;
    }
    ...
    game.ended = true;
    emitState(game);
    applyMoonveilGlyphResult(game, winner.username, 'timeout')
      ...
      .catch(err => {
        console.error('Moonveil Glyph points update error:', err);
        emitEnd(game, {
          winner: winner.username,
          reason: 'timeout',
          message: `${loser.username} ran out of time.`,
  ...
  function finishMoonveilGlyphGame(game, reason){
    const winner = getWinnerUsername(game);
    applyMoonveilGlyphResult(game, winner, winner ? reason : 'draw')
      .then(result=>emitEnd(game, {
        winner,
  ...
  function scheduleRoundAdvance(game){
    ...
    game.scoringTimer = setTimeout(()=>{
      game.scoringTimer = null;
      if(game.ended){
        finishMoonveilGlyphGame(game, 'game_end');
        return;
      }
  ...
  function findGameIdForSocket(){
    let gameId = state.moonveil_glyphPlayerGames[socket.id];
    if(gameId) return gameId;
    if(!socket.username) return null;

    gameId = Object.keys(state.moonveil_glyphGames).find(id =>
      state.moonveil_glyphGames[id]?.players?.some(p => p.username === socket.username)
    );

    if(gameId) state.moonveil_glyphPlayerGames[socket.id] = gameId;
    return gameId || null;
  }
  ...
  function emitState(game){
    ...
    game.players.forEach(player => {
      state.moonveil_glyphPlayerGames[player.id] = game.id;
      const s = io.sockets.sockets.get(player.id);
      if(!s) return;

      s.emit('moonveil_glyph_state', {
        gameId: game.id,
  ...
  function emitEnd(game, payload){
    game.players.forEach(player => {
      const s = io.sockets.sockets.get(player.id);
      if(s) s.emit('moonveil_glyph_end', payload);
    });
  }

  function findLobbyByUsername(username){
    return Object.values(state.moonveil_glyphLobbies).find(lobby =>
      lobby.players.some(player => player.username === username)
    ) || null;
  }

  function startLobbyGame(lobbyId){
    const lobby = state.moonveil_glyphLobbies[lobbyId];
    ...
    state.moonveil_glyphGames[gameId] = createGame(gameId, sourceLobby);
    delete state.moonveil_glyphLobbies[lobbyId];
    startTurnTimer(state.moonveil_glyphGames[gameId]);

    state.moonveil_glyphGames[gameId].players.forEach(p => {
      const s = io.sockets.sockets.get(p.id);
      if(s) s.emit('moonveil_glyph_start');
    });

    emitState(state.moonveil_glyphGames[gameId]);
    return true;
  }

  function findPublicMatch(maxPlayers){
    return randomEntry(Object.values(state.moonveil_glyphLobbies).filter(lobby =>
  ...
  function register(){
    socket.on('create_moonveil_glyph_lobby', ({ name, maxPlayers } = {})=>{
      if(isGameAllowed && !isGameAllowed('moonveil_glyph', socket)) return;
      ...
      const id = randomId();
      state.moonveil_glyphLobbies[id] = {
        id,
        name: name || 'Moonveil Glyph Room',
    ...
    socket.on('join_moonveil_glyph_lobby', id => {
      if(isGameAllowed && !isGameAllowed('moonveil_glyph', socket)) return;
      const lobby = state.moonveil_glyphLobbies[id];
    ...
    socket.on('toggle_moonveil_glyph_ready', id => {
      if(isGameAllowed && !isGameAllowed('moonveil_glyph', socket)) return;
      const lobby = state.moonveil_glyphLobbies[id];
      if(!lobby) return;
    ...
    socket.on('public_moonveil_glyph_matchmaking', ({ maxPlayers } = {}) => {
      if(isGameAllowed && !isGameAllowed('moonveil_glyph', socket)) return;
    ...
    socket.on('moonveil_glyph_move', ({ sourceType, sourceIndex, color, lineIndex } = {})=>{
    ...
    socket.on('moonveil_glyph_resign', ()=>{
      const gameId = findGameIdForSocket();
      if(!gameId) return;
      ...
      if(remaining.length > 1){
        ...
        game.players.forEach(player => {
          const s = io.sockets.sockets.get(player.id);
          if(s) s.emit('moonveil_glyph_notice', { message: `${quitter.username} resigned and was eliminated.` });
        });
        updatePresence();
        return;
      }
      ...
      game.ended = true;
      clearTurnTimer(game);
      applyMoonveilGlyphResult(game, winner.username, 'resign')
        .then(result=>emitEnd(game, {
          winner: winner.username,
          reason: 'resign',
    ...
    socket.on('moonveil_glyph_rematch', ()=>{
      const gameId = findGameIdForSocket();
      if(!gameId) return;
      ...
      game.players.forEach(player => {
        const s = io.sockets.sockets.get(player.id);
        if(!s) return;
        s.emit('moonveil_glyph_rematch_status', { requestedBy });
        if(player.username !== socket.username){
          s.emit('game_rematch_invite', {
            from: socket.username,
            toUsername: player.username,
            gameKey: 'moonveil_glyph',
            label: 'Moonveil Glyph',
            message: `${socket.username} wants an Moonveil Glyph rematch.`
          });
        }
      });
      ...
      state.moonveil_glyphGames[gameId] = createGame(gameId, lobby);
      state.rematchRequests[gameId] = {};
      startTurnTimer(state.moonveil_glyphGames[gameId]);

      emitState(state.moonveil_glyphGames[gameId]);
      state.moonveil_glyphGames[gameId].players.forEach(player => {
        const s = io.sockets.sockets.get(player.id);
        if(s) s.emit('moonveil_glyph_rematch_start');
      });
    });
  }
...
module.exports = { createMoonveilGlyphModule };
